...
import {
  ...
  getDriftActions,
  startDriftScan,
} from './drift-http-gateway';
import type { DriftResponse } from '../domain/drift';

const emptyResponse: DriftResponse = {
  available: true,
  enabled: true,
  readOnly: false,
  storageDriver: 'gcs',
...
describe('drift API query contract', () => {
  ...
  it('supplies and reuses an idempotency key with the current action contract', async () => {
    const fetchMock = vi.fn().mockImplementation(async (_url, init) => {
      const request = JSON.parse(String((init as RequestInit).body)) as Record<
        string,
        string
      >;
      return new Response(
    ...
    const action = await createDriftAction('plan-1');
    ...
    expect(action.results?.[1].logicPath).toBe('/failed');
    expect(action.progress).toBe(1);

    await createDriftAction('plan-1', action.idempotencyKey);
  ...
  it('returns the current plan contract unchanged', async () => {
    const response = {
      planId: 'plan-1',
      paths: ['report.pdf'],
      items: [
