...
import { ActivityDock } from '../../../shared/presentation/activity-dock';
import {
  Breadcrumbs,
  FolderMetric,
  TrashEmptyState,
} from './components/file-browser-chrome';
import { EmptyState, LoadingTable } from './components/file-browser-states';
import { FileTable } from './components/file-entry-views';
...
import { FILE_PAGE_SIZE, useFilesController } from './use-files-controller';
import {
  fileBrowserPath,
  DRIFT_ROUTE,
...
export function FilesPage({
  controllerDependencies: filesControllerDependencies,
  presentationDependencies: filesPresentationDependencies,
  uploadIntegration,
}: {
  controllerDependencies: FilesControllerDependencies;
  presentationDependencies: FilesPresentationDependencies;
  uploadIntegration: FilesUploadIntegration;
}) {
  ...
  const filesUploadQueue = uploadIntegration.useQueue();
  const UploadActivity = uploadIntegration.Activity;
  const UploadDialog = uploadIntegration.Dialog;
  useEffect(() => {
    const canonicalPath =
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-end xl:shrink-0">
            ...
            {view === 'files' && (
              <Button
                ...
                onFocus={() => void uploadIntegration.preloadDialog()}
                onPointerEnter={() => void uploadIntegration.preloadDialog()}
                className="h-9 w-full px-3 md:w-auto"
