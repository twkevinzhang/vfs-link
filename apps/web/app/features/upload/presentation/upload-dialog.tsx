...
import type { ArchiveBuildProgress, BuiltArchive } from './archive-compression';
import { buildArchivePlans, type ArchiveOptions } from './archive-plan';
...
export type { PreparedArchiveBatch } from './upload-candidates';
...
export function UploadDialog({
  ...
  >({});
  const [candidateRowLimit, setCandidateRowLimit] = useState(
    INITIAL_DIALOG_ROW_LIMIT
  );
  const [planRowLimit, setPlanRowLimit] = useState(INITIAL_DIALOG_ROW_LIMIT);
  ...
  const reset = useCallback(() => {
    ...
    setCandidateRowLimit(INITIAL_DIALOG_ROW_LIMIT);
    setPlanRowLimit(INITIAL_DIALOG_ROW_LIMIT);
  }, []);
  ...
  const visibleCandidates = useMemo(
    () => visibleDialogRows(candidates, candidateRowLimit),
    [candidateRowLimit, candidates]
  );
  const visiblePlans = useMemo(
    () => visibleDialogRows(plans, planRowLimit),
    [planRowLimit, plans]
  );
  ...
  const buildAndQueue = useCallback(async () => {
    ...
    try {
      const preparedThumbnails = await mapWithConcurrency(
        plans,
        4,
        async (plan, _index, signal) => {
          ...
          return firstDecodableThumbnail(ordered, signal);
        },
        controller.signal
      );
      const { buildArchives } = await import('./archive-compression');
              ...
              <div className="max-h-56 overflow-auto rounded-lg border border-border">
                ...
                {visibleCandidates.length < candidates.length && (
                  <Button
                    ...
                    onClick={() =>
                      setCandidateRowLimit((current) =>
                        nextDialogRowLimit(current, candidates.length)
                      )
                    }
                  >
                    再顯示{' '}
              ...
              <aside className="grid max-h-[58dvh] content-start gap-3 overflow-auto rounded-lg bg-muted/35 p-3">
                    ...
                    {plan.thumbnailCandidates.length > 0 ? (
                      <label className="grid gap-1 text-xs text-muted-foreground">
                        ...
                        <ThumbnailCandidateSelector
                          candidates={plan.thumbnailCandidates}
                          selectedPath={
                ...
                {visiblePlans.length < plans.length && (
                  <Button
                    ...
                    className="w-full rounded-none"
                    onClick={() =>
                      setPlanRowLimit((current) =>
                        nextDialogRowLimit(current, plans.length)
                      )
                    }
                  >
                    再顯示 {Math.min(100, plans.length - visiblePlans.length)}{' '}
                    個壓縮計畫
                  </Button>
                )}
              </aside>
