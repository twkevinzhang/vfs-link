...
METADATA_SHARD_COUNT="${METADATA_SHARD_COUNT:-64}"
METADATA_REDUCER_INTERVAL="${METADATA_REDUCER_INTERVAL:-2s}"
METADATA_MUTATION_MODE="${METADATA_MUTATION_MODE:-global}"
...
case "$METADATA_MUTATION_MODE" in
  global) ;;
  scoped)
    if [[ "$METADATA_PREFIX" != "_vfs-link-v4" ]]; then
      printf 'METADATA_MUTATION_MODE=scoped requires METADATA_PREFIX=_vfs-link-v4\n' >&2
      exit 1
    fi
    ;;
  *)
    printf 'METADATA_MUTATION_MODE must be global or scoped\n' >&2
    exit 1
    ;;
esac
if [[ ! "$METADATA_SHARD_COUNT" =~ ^[0-9]+$ ]]; then
  printf 'METADATA_SHARD_COUNT must be an integer\n' >&2
  exit 1
fi
if (( METADATA_SHARD_COUNT < 1 || METADATA_SHARD_COUNT > 256 || (METADATA_SHARD_COUNT & (METADATA_SHARD_COUNT - 1)) != 0 )); then
  printf 'METADATA_SHARD_COUNT must be a power of two between 1 and 256\n' >&2
  exit 1
fi
...
printf 'Metadata schema: %s; mutation mode: %s; shards: %s; reducer interval: %s\n' "$METADATA_PREFIX" "$METADATA_MUTATION_MODE" "$METADATA_SHARD_COUNT" "$METADATA_REDUCER_INTERVAL"
